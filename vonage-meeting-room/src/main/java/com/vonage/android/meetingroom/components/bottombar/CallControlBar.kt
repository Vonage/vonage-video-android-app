...
import com.vonage.android.meetingroom.components.bottombar.BottomBarTestTags.BOTTOM_BAR_CAMERA_BUTTON
import com.vonage.android.meetingroom.components.bottombar.BottomBarTestTags.BOTTOM_BAR_END_CALL_BUTTON
import com.vonage.android.meetingroom.components.bottombar.BottomBarTestTags.BOTTOM_BAR_MIC_BUTTON
...
fun CallControlBar(
    ...
    Row(
        ...
        if (allowMicrophoneControl) {
            MicButton(roomActions = roomActions, isMicEnabled = isMicEnabled)
        }
        if (allowCameraControl) {
            CameraButton(roomActions = roomActions, isCameraEnabled = isCameraEnabled)
        }
        content()
        MenuButton(onShowMore = onShowMore)
        EndCallButton(roomActions = roomActions)
    }
}

@Composable
private fun MicButton(roomActions: MeetingRoomActions, isMicEnabled: Boolean) {
    ControlButton(
        modifier = Modifier.testTag(BOTTOM_BAR_MIC_BUTTON),
...
private fun CameraButton(roomActions: MeetingRoomActions, isCameraEnabled: Boolean) {
    ControlButton(
        modifier = Modifier.testTag(BOTTOM_BAR_CAMERA_BUTTON),
...
private fun MenuButton(onShowMore: () -> Unit) {
    ControlButton(onClick = onShowMore, icon = VividIcons.Solid.MoreVertical)
}
