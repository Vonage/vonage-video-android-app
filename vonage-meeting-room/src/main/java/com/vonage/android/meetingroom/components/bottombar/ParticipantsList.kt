...
fun ParticipantsList(
    ...
    LazyColumn(
        ...
        if (sortedParticipants.isEmpty()) {
            ...
            items(items = sortedParticipants, key = { it.id }) { participant ->
                ParticipantRow(
                    participant = participant,
                    isPinned = participant.id in pinnedParticipantIds,
                    onTogglePin = { actions.onTogglePinParticipant(participant.id) },
                )
            }
        }
    }
...
private fun ParticipantListTitle(participantsCount: Int) {
    Text(
        ...
        text = stringResource(R.string.meeting_room_participants_list_title, participantsCount),
...
private fun ParticipantRow(
    ...
    Row(
        ...
        Text(
            modifier = Modifier.padding(end = VonageVideoTheme.dimens.paddingSmall).weight(1f),
        ...
        if (!participant.isPublisher) {
            IconButton(onClick = onTogglePin) {
                Icon(
                    imageVector = if (isPinned) VividIcons.Solid.Pin2Off else VividIcons.Solid.Pin2,
