...
class DefaultAudioDeviceStore(
    context: Context,
    private val audioManager: AudioManager = context.getSystemService(Context.AUDIO_SERVICE) as AudioManager,
    private val veraAudioDevice: VeraAudioDevice? = null,
) : AudioDeviceStore {
    private var selectedDevice: AudioDeviceInfo? = null

    override fun getDevices(): List<AudioDevice> =
    ...
    @SuppressLint("NewApi")
    override fun getActiveDevice(): AudioDevice? =
    ...
    @SuppressLint("NewApi")
    override fun selectDevice(device: AudioDevice): Boolean {
//        if (BuildConfigWrapper.sdkVersion() >= Build.VERSION_CODES.S) {
...
//                if (!result) {
//                    Log.e("XXX", "error while changing to $it")
//                    false
//                }
        ...
        Log.d("VeraAudioDeviceStore", "select device ${device.type}")
            when (device.type) {
                ...
                AudioDeviceType.HEADSET -> {
                    veraAudioDevice?.forceStopBluetooth()
                    audioManager.apply {
                        if (isBluetoothScoOn) { stopBluetoothSco() }
                ...
                AudioDeviceType.BLUETOOTH -> {
                    veraAudioDevice?.enableBluetoothManagement()
                    audioManager.apply {
